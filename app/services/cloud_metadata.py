...
class CloudMetadataService:
    ...
    def _get_gcp_metadata(self) -> Dict:
        ...
        try:
            ...
            # Parse zone (extract 'us-east1-b' from 'projects/.../zones/us-east1-b')
            region = zone_full.split('/')[-1] if zone_full else None
            
            # Parse machine type (extract 'e2-medium' from 'projects/.../machineTypes/e2-medium')
    ...
    def _get_gcp_network_interfaces(self, headers: Dict) -> List[Dict]:
        ...
        while True:
            try:
                ...
                # Check if interface exists by trying to get IP
                ...
                # Try to get public IP (may not exist)
                try:
                    public_ip = self._gcp_get(f"{base_path}/access-configs/0/external-ip", headers)
                except:
                    public_ip = None
    ...
    def _get_aws_network_interfaces(self) -> List[Dict]:
        ...
        try:
            ...
            for mac in macs:
                ...
                private_ip = private_ips_text.split('\n')[0].strip() if private_ips_text else None
                ...
                try:
                    ...
                    public_ip = public_ips_text.split('\n')[0].strip() if public_ips_text else None
                except:
                    public_ip = None
